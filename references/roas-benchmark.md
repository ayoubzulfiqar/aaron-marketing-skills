...
The paid-ads skills (phases: build · launch · scale) apply ROAS:

- **build** — [campaign-architect](../paid/campaign-architect/SKILL.md) scores **A** + structure; [ad-creative-builder](../paid/ad-creative-builder/SKILL.md) produces **O** units.
- **launch** — [ad-account-auditor](../paid/ad-account-auditor/SKILL.md) is the auditor-class gate: it scores RQS, enforces R1/R2/O1/O2/A1, and emits the [auditor-runbook](auditor-runbook.md) handoff schema to `memory/audits/paid/`.
- **scale** — [paid-measurement-loop](../paid/paid-measurement-loop/SKILL.md) reads ROAS/CPA against a control per [measurement-protocol.md](measurement-protocol.md), reusing `ledger.py` + `roi-calculator`.
