...
| **인플루언서** | 16 | scout → target → activate → report | [C³](../references/c3-benchmark.md) → `creator-content-auditor`(ART); `fit-scorer`가 ACE 채점 | `/aaron-marketing:influencer` |
...
| [creator-content-auditor](../influencer/activate/creator-content-auditor/SKILL.md) | C³ ART | `influencer/activate/` | SHIP / FIX / BLOCK / UNDECIDED에 더해 크리에이터용 번역 제공 |
...
| ⛩ creator-content-auditor | 크리에이터 제출물에 대한 게시 전 게이트 결정(C³ ART: FTC 공개 T1, 클레임 무결성 T2). |
...
3. **Activate** — `outreach-manager` → `creator-content-auditor`(⛩ ART 게이트) → `contract-helper` → `content-amplifier`
