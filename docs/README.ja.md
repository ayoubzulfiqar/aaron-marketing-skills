...
| **インフルエンサー** | 16 | scout → target → activate → report | [C³](../references/c3-benchmark.md) → `creator-content-auditor`（ART）；`fit-scorer` が ACE を採点 | `/aaron-marketing:influencer` |
...
| [creator-content-auditor](../influencer/activate/creator-content-auditor/SKILL.md) | C³ ART | `influencer/activate/` | SHIP / FIX / BLOCK / UNDECIDED に加えクリエイター向けの翻訳 |
...
| ⛩ creator-content-auditor | クリエイターの提出物への公開前ゲート判断（C³ ART：FTC 開示 T1、クレーム完全性 T2）。 |
...
4. **Evaluate** — `rank-tracker` → `performance-monitor` → `offsite-signal-analyzer`；信頼レビューには `domain-authority-auditor`（⛩）
...
3. **Activate** — `outreach-manager` → `creator-content-auditor`（⛩ ART ゲート） → `contract-helper` → `content-amplifier`
