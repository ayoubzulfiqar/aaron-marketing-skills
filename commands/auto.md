...
- Algorithm: parse the goal -> assign the closest scenario family -> attach risk gates -> choose the smallest `/aaron-marketing:seo-geo` mode (research / create / audit / track) or chain that completes the outcome -> ask only for blocking inputs -> continue to the natural stopping point.
